...
void CDirectXAppWiz::CustomizeProject(IBuildProject* pProject)
{
    ...
    for( i=1; i<lCount+1; i++ )
    {
        ...
        // Add d3dx lib
		if( wcsstr( bstrName, L"Debug" ) != NULL )
		{
			bstrSettings = SysAllocString( L"d3dx9dt.lib" );
			hr = pConfig->AddToolSettings( bstrTool, bstrSettings, var );
			SysFreeString( bstrSettings );
		}
		else
		{
			bstrSettings = SysAllocString( L"d3dx9.lib" );
			hr = pConfig->AddToolSettings( bstrTool, bstrSettings, var );
			SysFreeString( bstrSettings );
		}
        ...
        // Add all libs
        bstrSettings = SysAllocString( L"dsound.lib dinput8.lib dxerr9.lib d3d9.lib d3dxof.lib dxguid.lib winmm.lib kernel32.lib user32.lib gdi32.lib winspool.lib comctl32.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib" );
        hr = pConfig->AddToolSettings( bstrTool, bstrSettings, var );
        SysFreeString( bstrSettings );
