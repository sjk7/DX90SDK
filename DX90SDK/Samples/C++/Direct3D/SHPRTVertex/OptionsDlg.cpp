...
void UpdateConstText( HWND hDlg, DWORD dwMaxVertexShaderConst );
                                                    ...
                                                    WPARAM wParam, LPARAM lParam )
{
    switch( msg )
    {
        case WM_INITDIALOG:
        {
            ...
            HWND hNumClustersSpin = GetDlgItem( hDlg, IDC_NUM_CLUSTERS_SPIN );
            SendMessage( hNumClustersSpin, UDM_SETRANGE, 0, (LPARAM) MAKELONG( 200, 1 ) );
            SendMessage( hNumClustersSpin, UDM_SETPOS, 0, (LPARAM) MAKELONG( m_dwNumClusters, 0) );  
            ...
            _sntprintf( sz, 256, TEXT("%d"), m_d3dCaps.MaxVertexShaderConst );
            sz[255] = 0;
            SetDlgItemText( hDlg, IDC_MAX_CONSTS_EDIT, sz );

            UpdateConstText( hDlg, m_d3dCaps.MaxVertexShaderConst );
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDOK:
                {
                    DWORD dwNumClusters = (DWORD) SendMessage( GetDlgItem( hDlg, IDC_NUM_CLUSTERS_SPIN ), UDM_GETPOS, 0, 0 );
                    if( dwNumClusters > 200 || dwNumClusters < 1 )
                    {
                        TCHAR sz[256];
                        _tcsncpy( sz, TEXT("Number of clusters needs to be between 1 and 200"), 256 );
                        sz[255] = 0;
                        MessageBox( m_hWnd, sz, TEXT("Error"), MB_OK );
                        break;
                    }
                    m_dwNumClusters = dwNumClusters;
...
void UpdateConstText( HWND hDlg, DWORD dwMaxVertexShaderConst )
{
    ...
    DWORD dwNumClusters = (DWORD) SendMessage( GetDlgItem( hDlg, IDC_NUM_CLUSTERS_SPIN ), UDM_GETPOS, 0, 0 );
    if( dwNumClusters < 1 || dwNumClusters > 200 )
        dwNumClusters = 0xFFFF;
    ...
    DWORD dwNumVConsts = dwNumClusters * (1 + MAX_NUM_CHANNELS*dwNumPCAVectors/4) + 4;
    _sntprintf( sz, 256, TEXT("%d * (1 + (3*%d/4)) + 4 = %d"), dwNumClusters, dwNumPCAVectors, dwNumVConsts );
    sz[255] = 0;
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_VIEW_RESULTS:
                {
                    ...
                    else if( GetFileAttributes( szMesh ) == 0xFFFFFFFF )
...
void GetStartupToolTipText( int nDlgId, NMTTDISPINFO* pNMTDI )
{
    ...
    switch( nDlgId )
    {
        ...
        case IDC_WHAT: str = TEXT("This sample demonstrates how use D3DXSHPRTSimulation(), a vertex based SH PRT simulator and records the results to a file.  It can then view the results in result time with a vs_1_1 vertex shader."); break;
        ...
        case IDC_SIM_RESULTS_EDIT: str = TEXT("Previously saved results of the simulator for this mesh.  The simulator results are used in real time in the vertex shader to light the mesh.  If the results from a different mesh are used then either an error will occur or the lighting will be incorrect."); break;

        case IDC_SIM_RESULTS_BROWSE_BUTTON: str = TEXT("Select the simulation results file name"); break;

        case IDC_VIEW_RESULTS: str = TEXT("View the mesh with a vs_1_1 vertex shader using previously saved simulator results."); break;
...
void GetCompressToolTipText( int nDlgId, NMTTDISPINFO* pNMTDI )
{
    ...
    switch( nDlgId )
    {
        ...
        case IDC_PCA_VECTOR_COMBO: str = TEXT("The number of PCA vectors controls the amount of compression done after the simulator has run. There are order^2 transfer coefficients per channel. So for example, with order 6 and spectral turned on that would be 108 coefficients per vertex. To reduce the amount of data that needs to be passed to the vertex shader, D3DXSHPRTCompress() can be used to compress the data with a certain number of PCA vectors. The number of coefficients per vertex will be reduced to the number of PCA vectors. So, for example, with 20 PCA vectors and order 6 then instead of 108 coefficients per vertex we will only need 20 coefficients per vertex. The number of PCA vectors must be less than the order^2. For more detailed information, see \"Clustered Principal Components for Precomputed Radiance Transfer\" by Peter-Pike Sloan, Jesse Hall, John Hart, and John Snyder to appear in the Proceedings of SIGGRAPH 2003"); break;

        case IDC_NUM_CLUSTERS_TEXT: 
        case IDC_NUM_CLUSTERS_EDIT: str = TEXT("The number of clusters is some number of vertices that share a common mean vector. The more clusters yields more accurature the results with the tradeoff of more shader constants required. For more detailed information, see \"Clustered Principal Components for Precomputed Radiance Transfer\" by Peter-Pike Sloan, Jesse Hall, John Hart, and John Snyder to appear in the Proceedings of SIGGRAPH 2003"); break;

        case IDC_MAX_CONSTS_EDIT: 
        case IDC_MAX_CONSTS_TEXT: str = TEXT("This is the maximum number of constant supported by this HW device for a vs_1_1 shader.  This is simply d3dCaps.MaxVertexShaderConst"); break;

        case IDC_NUM_CONSTS_EDIT: 
        case IDC_NUM_CONSTS_TEXT: str = TEXT("This is the number of vertex shader constants that will be needed by the sample's shader given the current dialog settings.  The formula is dwNumClusters*(1+MAX_NUM_CHANNELS*dwNumPCAVectors/4)+4."); break;
    }
