//-----------------------------------------------------------------------------
// File: SHPRTVertex.cpp
//
// Desc: This sample demonstrates how use D3DXSHPRTSimulation(), a per vertex  
...
{
    m_strWindowTitle = _T("SHPRTVertex");
    ...
    DXUtil_ReadIntRegKey( hRegKey, TEXT("NumRays"), &m_dwNumRays, 1024 );
    ...
    DXUtil_ReadIntRegKey( hRegKey, TEXT("NumClusters"), &m_dwNumClusters, 10 );   
...
//-----------------------------------------------------------------------------
// Name: DoCompressDlg()
// Desc: Allows the user to change the # of PCA vectors and the # of clusters.
//-----------------------------------------------------------------------------
...
HRESULT CMyD3DApplication::CompressData()
{
    ...
    if( dwNumVConsts > m_d3dCaps.MaxVertexShaderConst )
    {
    ...
    if( m_pMesh->GetNumVertices() != desc.NumSamples )
    {
    ...
    hr = SetupVBWithPCAData( m_pMesh, pCPCABuffer, desc, m_dwNumClusters, m_dwNumPCAVectors );
    if( FAILED(hr) )    
...
//       data to the shader
//-----------------------------------------------------------------------------
HRESULT CMyD3DApplication::AdjustMeshDecl( ID3DXMesh** ppMesh )
{
    HRESULT hr;
...
//-----------------------------------------------------------------------------
HRESULT CMyD3DApplication::SetupVBWithPCAData( ID3DXMesh* pMesh,
...
HRESULT CMyD3DApplication::LoadSHPRTEffect()
{
    ...
    // Read the D3DX effect file
    TCHAR str[MAX_PATH];
    DXUtil_FindMediaFileCb( str, sizeof(str), TEXT("SHPRTVertex.fx") );
...
HRESULT CMyD3DApplication::RenderText()
{
    ...
    DWORD dwNumVConsts = m_dwNumClusters * (1 + MAX_NUM_CHANNELS*m_dwNumPCAVectors/4) + 4;
    _sntprintf( sz, 256, TEXT("# clusters: %d, # PCA vectors: %d, # Constants (%d max): %d"), m_dwNumClusters, m_dwNumPCAVectors, m_d3dCaps.MaxVertexShaderConst, dwNumVConsts );
    SetRect( &rc, 2, iLineY, 0, 0 ); iLineY += 15;
    hr = m_pFont->DrawText( NULL, sz, -1, &rc, DT_NOCLIP, D3DXCOLOR( 1.0f, 1.0f, 1.0f, 1.0f ) );
    if( FAILED(hr) )
        return DXTRACE_ERR( TEXT("DrawText"), hr );
    ...
    if( m_bSHPRTTooManyConstants )
    {
        _sntprintf( sz, 256, TEXT("Too many constants needed! Max: %d, Cur: %d\n"), m_d3dCaps.MaxVertexShaderConst, dwNumVConsts );
        sz[255] = 0;
        SetRect( &rc, 2, iLineY, 0, 0 ); iLineY += 15;
        hr = m_pFont->DrawText( NULL, sz, -1, &rc, DT_NOCLIP, D3DXCOLOR( 1.0f, 0.0f, 0.0f, 1.0f ) );
        if( FAILED(hr) )
    ...
    else
    {
        switch( m_nTechnique )
        {
            case 0: 
                if( m_bSHPRTReadyToRender )
                    lstrcpy( sz, TEXT("This is per vertex PRT.  Press 'N' to change to simple N.L lighting") ); 
                else
                    lstrcpy( sz, TEXT("Simulation in progress.  Press 'N' to change to simple lighting") ); 
                break;
            case 1: 
                if( m_bSHPRTReadyToRender )
                    lstrcpy( sz, TEXT("This is simple N.L lighting.  Press 'N' to change to per vertex PRT") ); 
                else
    ...
    if( m_bShowHelp )
    {
        ...
        hr = m_pFont->DrawText( NULL, _T("Change active light: K\n")
                                      ...
                                      _T("Compression settings: C,E,R,D,F\n")
...
HRESULT CMyD3DApplication::FinalCleanup()
{
    ...
    DXUtil_WriteIntRegKey( hRegKey, TEXT("NumClusters"), m_dwNumClusters );   
...
DWORD CMyD3DApplication::PRTSimulationThreadProc()
{
    ...
    D3DXSHMATERIAL* pMatPtr[1] = { &shMat };
    hr = D3DXSHPRTSimulation( m_dwOrder, 1, &m_pMesh, pMatPtr, 
                              m_dwNumRays, m_dwNumBounces, m_bSubsurfaceScattering,
                              m_fLengthScale, m_bSpectral, &m_pSHPRTBuffer, StaticSHPRTSimulatorCB );
    if( FAILED(hr) )       
    {
    ...
    switch( uMsg )
    {
        ...
        case WM_KEYDOWN:
            switch( wParam )
            {
                ...
                case 'D': m_dwNumClusters++; if( m_dwNumClusters > 200 ) m_dwNumClusters = 200; CompressData(); break;
                case 'F': m_dwNumClusters--; if( m_dwNumClusters < 1 ) m_dwNumClusters = 1; CompressData(); break;
            }
            break;
