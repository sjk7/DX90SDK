...
HRESULT C3DModel::InitDeviceObjects( TCHAR* strModelName, const BOOL bCreateTest, float fBoundingScale ) 
{
    ...
    if( bCreateTest ) 
    {
        ...
        if( NULL == m_pMesh->m_pMaterials )
            return E_OUTOFMEMORY;
        ZeroMemory( &m_pMesh->m_pMaterials[0], sizeof(D3DMATERIAL9) );
        m_pMesh->m_pMaterials[0].Diffuse.r = m_pMesh->m_pMaterials[0].Ambient.r = 1.0f;
        m_pMesh->m_pMaterials[0].Diffuse.g = m_pMesh->m_pMaterials[0].Ambient.g = 0.0f;
        m_pMesh->m_pMaterials[0].Diffuse.b = m_pMesh->m_pMaterials[0].Ambient.b = 0.0f;
        m_pMesh->m_pMaterials[0].Diffuse.a = m_pMesh->m_pMaterials[0].Ambient.a = 1.0f;
