...
HRESULT CMyD3DApplication::FrameMove()
{
    ...
    D3DXMatrixRotationX( &matWorld, cosf(m_fTime/6.0f) );
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
        ...
        if( m_dwNumIntersections > 0 )
        {
            for( DWORD iIntersection = 0; iIntersection < m_dwNumIntersections; iIntersection++ )
            {
                ...
                _stprintf( strBuffer, _T("Face=%d, tu=%3.02f, tv=%3.02f"), pIntersection->dwFace, pIntersection->tu, pIntersection->tv );
        ...
        {
            _stprintf( strBuffer, _T("Use mouse to pick a polygon") );
            m_pFontSmall->DrawText( 2, yText, D3DCOLOR_ARGB(255,0,255,255), strBuffer );
        }
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    D3DLIGHT9 light;
    D3DXVECTOR3 vecLightDirUnnormalized(0.1f, -1.0f, 0.1f);
                                    ...
                                    LPARAM lParam )
{
    switch( msg )
    {
        ...
        case WM_COMMAND:
            if( LOWORD(wParam) == IDM_SHOW_HELP )
