...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    if( FAILED( hr = DXUtil_FindMediaFileCb( strPath, sizeof(strPath), TEXT("Fur.fx") ) ) )
        return hr;
    if( FAILED( hr = D3DXCreateEffectFromFile( m_pd3dDevice, strPath, NULL, NULL, dwShaderFlags, NULL, &m_pEffect, NULL) ) )
        return hr;
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    for( UINT i = 0; i<m_nShells; i++ )
    {
        ...
        if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), szFileName ) ) )
            return D3DAPPERR_MEDIANOTFOUND;
        ...
        if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                         ...
                         D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, m_pShellTextures + i ) ) )
            return D3DAPPERR_MEDIANOTFOUND;
    }

    // Get the path to the texture
    if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("fur_fin_00.dds") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    // create the fin texture
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                        D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_A8R8G8B8, 
                        D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                        D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pFinTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
