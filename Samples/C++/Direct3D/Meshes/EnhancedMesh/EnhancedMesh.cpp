...
HRESULT CMyD3DApplication::Render()
{
    ...
    if( SUCCEEDED( m_pd3dDevice->BeginScene() ) )
    {
        ...
        // Output stats
        {
            ...
            _stprintf( buffer1, _T("NumSegs:") ); 
            _stprintf( buffer2, _T("%d "), m_dwNumSegs );
            m_pFont->DrawText(   2, 40, 0xffffff00, buffer1 );
            m_pFont->DrawText( 150, 40, 0xffffffff, buffer2 );
            _stprintf( buffer1, _T("NumFaces:") );
            _stprintf( buffer2, _T("%d "), (m_pMeshEnhanced == NULL) ? 0 : m_pMeshEnhanced->GetNumFaces() );
            m_pFont->DrawText(   2, 60, 0xffffff00, buffer1 );
            m_pFont->DrawText( 150, 60, 0xffffffff, buffer2 );
            _stprintf( buffer1, _T("NumVertices:") );
            _stprintf( buffer2, _T("%d "), (m_pMeshEnhanced == NULL) ? 0 : m_pMeshEnhanced->GetNumVertices() );
        ...
        if( m_bShowHelp )
        {
            m_pFont->DrawText( 2, 100, D3DCOLOR_ARGB(255,255,255,255),
                                    _T("Keyboard controls:"));
            m_pFont->DrawText( 20, 120, D3DCOLOR_ARGB(255,255,255,255),
                                    _T("Add Segments\nRemove Segments\nWireframe\n")
                                    _T("Solid\n")
                                    _T("Help\nChange Device\nQuit"));
            m_pFont->DrawText( 240, 120, D3DCOLOR_ARGB(255,255,255,255),
                                    _T("Up Arrow\nDown Arrow\nW\n")
                                    _T("S\nF1\nF2\nEsc"));
        }
        else
        {
            m_pFont->DrawText( 2, 100, D3DCOLOR_ARGB(255,255,255,255), _T("Press F1 for help"));
        }
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    for( UINT i=0; i<m_dwNumMaterials; i++ )
    {
        ...
        tszName = tszBuf;
        MultiByteToWideChar( CP_ACP, 0, m_pMaterials[i].pTextureFilename, -1, tszBuf, MAX_PATH );
        tszBuf[MAX_PATH - 1] = _T('\0');
#else
        tszName = m_pMaterials[i].pTextureFilename;
#endif
        DXUtil_FindMediaFileCb( strTexturePath, sizeof(strTexturePath), tszName );
