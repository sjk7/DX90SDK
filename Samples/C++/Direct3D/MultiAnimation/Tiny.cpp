                           ...
                           double dTimeCurrent )
{
    ...
    {
        hr = DXUtil_FindMediaFileCch( sPath, MAX_PATH, FOOTFALLSOUND00 );
        if( FAILED( hr ) )
            _tcsncpy( sPath, FOOTFALLSOUND00, MAX_PATH );

        hr = m_pSM->Create( & g_apSoundsTiny[ 0 ], sPath, DSBCAPS_CTRLVOLUME );
        if( FAILED( hr ) )
        {
            OutputDebugString( FOOTFALLSOUND00 _T(" not found; continuing without sound.\n") );
            m_bPlaySounds = false;
        }
    }
    ...
    {
        hr = DXUtil_FindMediaFileCch( sPath, MAX_PATH, FOOTFALLSOUND01 );
        if( FAILED( hr ) )
            _tcsncpy( sPath, FOOTFALLSOUND01, MAX_PATH );

        hr = m_pSM->Create( & g_apSoundsTiny[ 1 ], sPath, DSBCAPS_CTRLVOLUME );
        if( FAILED( hr ) )
        {
            OutputDebugString( FOOTFALLSOUND01 _T(" not found; continuing without sound.\n") );
            m_bPlaySounds = false;
        }
    }
...
void CTiny::Report( vector < String > & v_sReport )
{
    ...
    try
    {
        _stprintf( s, _T("Pos: %f, %f"), m_vPos.x, m_vPos.z );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("Facing: %f"), m_fFacing );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("Local time: %f"), m_dTimeCurrent );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("Pos Target: %f, %f"), m_vPosTarget.x, m_vPosTarget.z );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("Facing Target: %f"), m_fFacingTarget );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("Status: %s"), m_bIdle ? _T("Idle") : ( m_bWaiting ? _T("Waiting") : _T("Moving") ) );
        v_sReport.push_back( String( s ) );
        ...
        _stprintf( s, _T("Track 0: %s%s"), pAS->GetName(), m_dwCurrentTrack == 0 ? _T(" (current)") : _T("") );
        v_sReport.push_back( String( s ) );
        pAS->Release();

        pAC->GetTrackDesc( 0, & td );
        _stprintf( s, _T("  Weight: %f"), td.Weight );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("  Speed: %f"), td.Speed );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("  Position: %f"), td.Position );
        v_sReport.push_back( String( s ) );
        _stprintf( s, _T("  Enable: %s"), td.Enable ? _T("true") : _T("false") );
        v_sReport.push_back( String( s ) );

        pAC->GetTrackAnimationSet( 1, & pAS );
        _stprintf( s, _T("Track 1: %s%s"), pAS->GetName(), m_dwCurrentTrack == 1 ? _T(" (current)") : _T("") );
        v_sReport.push_back( String( s ) );
        pAS->Release();
        ...
        if( m_bUserControl )
        {
            _stprintf( s, _T("Control: USER") );
            v_sReport.push_back( String( s ) );
        } else
        {
            _stprintf( s, _T("Control: AUTO") );
            v_sReport.push_back( String( s ) );
        }
...
// Desc: Specifies whether this instance of CTiny is controlled by the
//       user or the program.
//-----------------------------------------------------------------------------
