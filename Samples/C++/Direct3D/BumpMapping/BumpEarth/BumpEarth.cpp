...
class CMyD3DApplication : public CD3DApplication
{
    ...
    CModelViewerCamera m_Camera;            // Camera used for mouse input
...
HRESULT CMyD3DApplication::FrameMove()
{
    ...
    if( FALSE == m_Camera.IsBeingDragged() )
        fRotationAngle += m_fElapsedTime;

    // Setup viewing postion from ArcBall
    D3DXMatrixRotationY( &mRoll, -fRotationAngle );

    mCamWorld = *m_Camera.GetWorldMatrix();

    m_matWorld = mRoll * mCamWorld;
...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("Block.bmp") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    // Create the texture using D3DX
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                     D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_R5G6B5, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pBlockTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;

    // Get the path to the texture
    if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("Earth.bmp") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    // Create the texture using D3DX
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                     D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_R5G6B5, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pEarthTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;

    // Get the path to the texture
    if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("Earthbump.bmp") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    // Create the texture using D3DX
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                     D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_A8R8G8B8, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pEarthBumpTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;

    // Get the path to the texture
    if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("EarthEnvMap.bmp") ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    // Create the texture using D3DX
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                     D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_R5G6B5, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pEnvMapTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
...
HRESULT CMyD3DApplication::RestoreDeviceObjects()
{
    ...
    m_Camera.SetProjParams( D3DX_PI/4, fAspect, 0.1f, 225.0f );
    m_Camera.SetWindow( m_d3dsdBackBuffer.Width,  m_d3dsdBackBuffer.Height );
    m_matProj = *m_Camera.GetProjMatrix();
    ...
    m_Camera.SetRadius( 5.0f, 2.0f, 10.0f );
