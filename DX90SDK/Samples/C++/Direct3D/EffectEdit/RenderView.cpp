    ...
    if( SUCCEEDED( hr = m_pd3dDevice->BeginScene() ) )
    {
        if( m_pEffect != NULL )
        {
            if (m_pEnvironmentTexture != NULL)
            {
                ...
                m_pd3dDevice->SetTransform( D3DTS_WORLD, &matWorldEnvMap);
                m_pd3dDevice->SetTransform( D3DTS_VIEW, m_ArcBallView.GetRotationMatrix() );

                // Render background image
                m_pd3dDevice->SetTexture(0, m_pEnvironmentTexture);
                ...
                m_pd3dDevice->SetFVF( CUBEBACKGROUNDVERTEX::FVF);
                m_pd3dDevice->DrawIndexedPrimitiveUP( D3DPT_TRIANGLELIST, 0, 8, 12, EnvironmentIndices, 
            ...
            else if( m_pBackgroundTexture != NULL )
            {
                ...
                m_pd3dDevice->SetFVF( FLATBACKGROUNDVERTEX::FVF);
        ...
        if( m_bShowStats )
        {
            ...
            m_pFont->DrawText( 2, 15, D3DCOLOR_ARGB(255,255,255,0), m_strDeviceStats );
            if( m_pEffect == NULL )
                m_pFont->DrawText( 2, 30, D3DCOLOR_XRGB(255,0,0), TEXT("Error in effect file") );
            ...
            if( !m_bTechniqueValid )
                m_pFont->DrawText( 2, 45, D3DCOLOR_ARGB(255, 255, 0, 0), TEXT("Warning: technique not valid with current device settings"));

            if( !((CEffectEditApp*)AfxGetApp())->AppActivated() )
            {
                m_pFont->DrawText( 2, 60, D3DCOLOR_ARGB(255, 255, 255, 255), TEXT("Note: sleeping between frames because app is not frontmost, fps may be lower"));
            }
        }
        // End the scene.
...
HRESULT CRenderView::RestoreDeviceObjects()
{
    ...
    if ( FAILED( hr = m_pd3dDevice->CreateVertexBuffer( sizeof(FLATBACKGROUNDVERTEX)*4, 0,
    ...
    m_ArcBallMesh.SetWindow( m_d3dsdBackBuffer.Width, m_d3dsdBackBuffer.Height, 0.85f );
    m_ArcBallView.SetWindow( m_d3dsdBackBuffer.Width, m_d3dsdBackBuffer.Height, 0.85f );
...
HRESULT CRenderView::ParseParameters(CString &strErrors)
{
        ...
        Desc.Type == D3DXPT_STRING )
    {
        ...
        TCHAR strPath[MAX_PATH];
        EE_FindMediaFileCb( strPath, sizeof(strPath), strBackTexture );
        if( FAILED( hr = D3DXCreateCubeTextureFromFile( m_pd3dDevice, strPath, &m_pEnvironmentTexture) ) )
        {
    ...
    for( UINT iParam = 0; iParam < m_EffectDesc.Parameters; iParam++ )
    {
            ...
            ( ParamDesc.Class == D3DXPC_VECTOR ))
        {
            ...
            else if( strcmpi( ParamDesc.Semantic, "cameraposition" ) == 0 )
        ...
        else if( pstrFunction != NULL )
        {
            ...
            if( SUCCEEDED( hr = D3DXCompileShader( strEffectA, (UINT)strlen(strEffectA), NULL, 
                &includeManager, pstrFunction, pstrTarget, 0, &pTextureShader, &pBufferErrors, NULL ) ) )
            {
        ...
        SUCCEEDED( m_pEffect->GetParameterDesc( "XFile", &Desc ) ) )
    {
        ...
        if( strcmpi( fileName, m_strMesh ) != 0 || m_pMesh == NULL )
        {
            UnloadMesh();
...
DoneLooking:
    if( bUsesTangents && m_pMesh != NULL )
    {
        ...
        if( !bHasTangents ) 
        {
            ...
            hr = m_pMesh->CloneMesh( D3DXMESH_MANAGED, Declaration, m_pd3dDevice, &pTempMesh );
            if( SUCCEEDED( hr ) )
            {
