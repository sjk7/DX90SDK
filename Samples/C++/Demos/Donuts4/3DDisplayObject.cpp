...
{
    // Remove this object from the audiopath list otherwise, 
    // there'll be a dangling pointer to this object in that list
    for( int i=0; i<MAX_AUDIOPATHS; i++ )
    {
        if( g_pApp->m_AudioPath[i].m_pObject == this )
            g_pApp->m_AudioPath[i].m_pObject = NULL;
    }           
