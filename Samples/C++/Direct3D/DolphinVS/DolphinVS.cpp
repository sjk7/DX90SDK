...
HRESULT CMyD3DApplication::InitDeviceObjects()
{
    ...
    if( FAILED( hr = DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("Dolphin.bmp") ) ) )
        return hr;
    // Create texture for the dolphin
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
        D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pDolphinTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;

    // Get the path to the texture
    if( FAILED( hr = DXUtil_FindMediaFileCb( strPath, sizeof(strPath), _T("SeaFloor.bmp") ) ) )
        return hr;
    // Create textures for the seafloor
    if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
        D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, 
                     D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                     D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pSeaFloorTexture ) ) )
        return D3DAPPERR_MEDIANOTFOUND;
    ...
    for( DWORD t=0; t<32; t++ )
    {
        ...
        // Get the path to the texture
        if( FAILED( DXUtil_FindMediaFileCb( strPath, sizeof(strPath), strName ) ) )
            return D3DAPPERR_MEDIANOTFOUND;
        if( FAILED( D3DXCreateTextureFromFileEx( m_pd3dDevice, strPath, 
                         D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, 
                         D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 
                         D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, 0, NULL, NULL, &m_pCausticTextures[t] ) ) )
            return D3DAPPERR_MEDIANOTFOUND;
    }
