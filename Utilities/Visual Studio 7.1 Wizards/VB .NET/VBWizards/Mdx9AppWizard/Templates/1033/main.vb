...
[!if !GRAPHICSTYPE_DIRECT3D]
    ...
    '/ </summary>
    Private Sub MainClass_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
...
[!if GRAPHICSTYPE_NOGRAPHICS || GRAPHICSTYPE_DIRECTDRAW && !VISUALTYPE_BLANK]
    ...
    Private Sub CheckBounds()
        If (destination.X < 0) Then
            destination.X = 0
        End If
        If (destination.X > target.ClientSize.Width - spriteSize) Then
            destination.X = target.ClientSize.Width - spriteSize
        End If
        If (destination.Y < 0) Then
            destination.Y = 0
        End If
        If (destination.Y > target.ClientSize.Height - spriteSize) Then
